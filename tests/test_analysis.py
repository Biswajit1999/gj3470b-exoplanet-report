...
def test_pipeline_reproduces_committed_statistics():
    analysis.main()
    values = read_summary()
    assert np.isclose(values["depth_ppm"], 5574.20365737, rtol=0, atol=0.1)
    assert np.isclose(values["chi_square_flat"], 9418.55220849, rtol=1e-9)
    assert np.isclose(values["chi_square_box"], 2824.42736526, rtol=1e-9)
    assert np.isclose(values["delta_chi_square"], 6594.12484323, rtol=1e-9)
